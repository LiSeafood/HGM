...
class HGM(nn.Module):
    def __init__(self, in_dim, hid_dim=128, out_dim=32, proj_dim=32):
        ...
        # 对比学习投影头(常见做法：encoder输出 -> projection space)。需不需要改中间层维度呢？
    ...
    def forward(self, x, shg, fhg):
        ...
        z = zs + zf
        # 自适应加权融合
        # z_stack = torch.stack([zs, zf], dim=1)
        # z, att = self.att(z_stack)
        # self.att = att # 可以把注意力权重保存下来，打印查看空间和特征哪个更重要
        x_hat = self.decoder(z)
        # 投影到对比学习空间, 用于后续对比学习
        zs_pro = self.proj_spatial(zs)
        zf_pro = self.proj_feature(zf)
        return z, zs_pro, zf_pro, x_hat
